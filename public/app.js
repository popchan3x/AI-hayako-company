...
const elements = {
  ...
  schedulerLastResult: document.querySelector("#schedulerLastResult"),
  schedulerLastRun: document.querySelector("#schedulerLastRun"),
  schedulerLastAdded: document.querySelector("#schedulerLastAdded"),
  schedulerReport: document.querySelector("#schedulerReport"),
  schedulerError: document.querySelector("#schedulerError"),
  archiveStatus: document.querySelector("#archiveStatus"),
  archivePeriod: document.querySelector("#archivePeriod"),
  archiveReports: document.querySelector("#archiveReports"),
  archiveRows: document.querySelector("#archiveRows"),
  archiveDuplicates: document.querySelector("#archiveDuplicates"),
  archiveRecentReports: document.querySelector("#archiveRecentReports"),
...
function renderScheduler(scheduler) {
  elements.schedulerStatus.textContent = scheduler.running
    ? `実行中（${TIMEFRAME_LABELS[scheduler.currentInterval] || scheduler.currentInterval || "準備中"}）`
    : scheduler.enabled ? "有効" : "停止";
  ...
  const resultLabels = { success: "成功", failed: "失敗", running: "実行中" };
  elements.schedulerLastResult.textContent = resultLabels[scheduler.lastRunStatus] || "未実行";
  elements.schedulerLastRun.textContent = scheduler.lastFinishedAt
    ? new Date(scheduler.lastFinishedAt).toLocaleString("ja-JP")
    : "-";
  elements.schedulerLastAdded.textContent = scheduler.lastRunTotals
    ? `シグナル ${scheduler.lastRunTotals.newSignals || 0} / 答え ${scheduler.lastRunTotals.newOutcomes || 0}`
    : "-";
  elements.schedulerReport.textContent = scheduler.lastReport || "-";
  elements.schedulerError.hidden = !scheduler.lastError;
  elements.schedulerError.textContent = scheduler.lastError ? `最終エラー: ${scheduler.lastError}` : "";
}

function renderLearningArchive(archive) {
  elements.archiveStatus.textContent = archive.integrated ? "統合済み" : "未統合";
  elements.archiveStatus.classList.toggle("is-ready", archive.integrated);
  elements.archivePeriod.textContent = archive.reports.firstDate && archive.reports.lastDate
    ? `${archive.reports.firstDate} 〜 ${archive.reports.lastDate}`
    : "-";
  elements.archiveReports.textContent = `${archive.reports.count}件`;
  elements.archiveRows.textContent = `シグナル ${archive.signals.rows} / 答え ${archive.outcomes.rows}`;
  elements.archiveDuplicates.textContent = `シグナル ${archive.signals.duplicates} / 答え ${archive.outcomes.duplicates}`;
  listItems(
    elements.archiveRecentReports,
    archive.reports.recent.map((report) => `${report.date}　${report.path}`)
  );
}
...
async function loadLearningArchive() {
  const payload = await requestJson("/api/learn/archive");
  renderLearningArchive(payload.archive);
}

async function runDailyLearning() {
  elements.learnButton.disabled = true;
  setStatus("統合日次学習中（6時間足）");
  try {
    const payload = await requestJson("/api/learn/daily", { method: "POST" });
    renderLearning(payload.summary);
    renderScheduler(payload.scheduler);
    const added = payload.scheduler.lastRunTotals?.newSignals || 0;
    setStatus(`統合日次学習完了 ${added}件追加`);
  } catch (error) {
    setStatus("学習失敗");
    elements.explanation.textContent = error.message;
  } finally {
