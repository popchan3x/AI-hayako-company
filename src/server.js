...
import { readLearningSummary } from "./learning.js";
import { readLearningArchive } from "./learningArchive.js";
...
async function handleApi(request, response) {
  ...
  if (request.method === "GET" && url.pathname === "/api/learn/archive") {
    sendJson(response, 200, { ok: true, archive: await readLearningArchive() });
    return;
  }
  if (request.method === "GET" && url.pathname === "/api/learn/scheduler") {
    await learningScheduler.ready;
    sendJson(response, 200, { ok: true, scheduler: learningScheduler.state });
    return;
  }
  if (request.method === "POST" && url.pathname === "/api/learn/daily") {
    sendJson(response, 200, { ok: true, summary: await learningScheduler.runNow(), scheduler: learningScheduler.state });
    return;
  }
